...
export default function CommunicationSection({ motionConfig }) {
  return (
    <AnimatedSection
      ...
      background="bg-[radial-gradient(circle_at_20%_40%,rgba(0,102,255,0.13),transparent_30%),radial-gradient(circle_at_82%_55%,rgba(0,102,255,0.13),transparent_32%)]"
    >
      <div className="relative mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        <motion.div variants={fadeUp} transition={motionConfig.transition}>
          <p className="text-sm font-black uppercase tracking-[0.22em] text-sky-200">Communication client</p>
        ...
        <motion.div variants={stagger} className="grid gap-5 md:grid-cols-3 lg:grid-cols-1">
          {communication.map((item) => (
            <motion.div
              ...
              className={`rounded-2xl p-6 transition hover:border-blue-500/35 hover:bg-white/[0.08] ${glassBase}`}
            >
              <div className="flex gap-4">
                <div className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-blue-500/10 text-sky-200">
                  <Icon name={item.icon} />
                </div>
                <div>
...
function ClientSteps() {
  return (
    <div className="mt-8 rounded-2xl border border-white/10 bg-[#0d1118]/85 p-5 backdrop-blur-xl">
      <p className="text-sm font-black uppercase tracking-[0.18em] text-sky-200">Votre parcours</p>
      <div className="mt-5 grid gap-3">
        {clientSteps.map((step, index) => (
          <div key={step} className="flex items-center gap-3 rounded-2xl bg-slate-950/60 px-4 py-3">
            <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-blue-500 text-xs font-black text-white">
              {index + 1}
            </span>
