...
export default function Process({ motionConfig }) {
  return (
    <LandingSection id="process" eyebrow="Comment ça marche ?" title="Un accompagnement simple, du premier brief à la mise en ligne">
      <div className="mt-7 grid justify-items-center gap-5 lg:grid-cols-4">
        {landingSteps.map(([number, icon, title, text], index) => (
          <motion.article
            ...
            className="relative flex max-w-xs flex-col items-center text-center"
          >
            ...
            <div className="mt-5">
              ...
              <p className="mx-auto mt-2 max-w-xs text-xs leading-6 text-slate-400">{text}</p>
            </div>
            {index < landingSteps.length - 1 && (
              <div className="absolute left-[calc(50%+3.5rem)] top-6 hidden h-px w-40 border-t border-dashed border-blue-300/25 lg:block" />
            )}
          </motion.article>
        ))}
      </div>
